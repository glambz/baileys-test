...
import { buildSystemPrompt, buildSystemPromptFragment } from '../ai/settings/composer.js';
import { DEFAULT_AI_SETTINGS } from '../ai/settings/defaults.js';
import { BAILEYS_AI_SYSTEM_PROMPT_ID, BAILEYS_AI_SYSTEM_PROMPT_EN } from '../ai/llm/base-prompts.js';
import { getHardenedRulesBlock } from '../ai/settings/hardened-rules.js';
...
describe('composer byte identity (BE mirrors FE)', () => {
  ...
  it('BE base prompts match FE source byte-for-byte when FE source is available', () => {
    ...
    const beSrc = fs.readFileSync(path.join(__dirname, '..', 'ai', 'llm', 'base-prompts.js'), 'utf8');
