...
async function send(req, res, next) {
  try {
    ...
    try {
      ...
      // BUG-PUSH-EVENTS feature (2026-08-03): emit message.created so the
      // FE's SSE stream receives the new outbound message in real time.
      try {
        const events = require('../api/events');
        events.publish(jid, 'message.created', {
          id: sent?.key?.id,
          chatId: jid,
          direction: 'out',
