...
export function useSendMessage(chatId: string) {
  ...
  return useMutation<Message, Error, SendMessageInput, { previous: Message[] | undefined }>({
    ...
    onMutate: async ({ body }) => {
      ...
      const previous = qc.getQueryData<Message[]>(['messages', chatId]);
      ...
      qc.setQueryData<Message[]>(['messages', chatId], (old) => [...(old ?? []), optimistic]);
